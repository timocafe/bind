...
namespace ambient {
    ...
    void for_each(InputIterator begin, InputIterator end, Function fn){
        ...
        for(iterator bit(begin,end); bit != end; ++bit)
            ambient::bind([fn](typename iterator::block_type& block, size_t first, size_t second){
    ...
    void sequence(InputIterator begin, InputIterator end){
        ...
        for(iterator bit(begin,end); bit != end; ++bit){
            ...
            ambient::bind([offset](typename iterator::block_type& block, size_t first, size_t second){
    ...
    void fill(ForwardIterator begin, ForwardIterator end, const T& val){
        ...
        for(iterator bit(begin,end); bit != end; ++bit){
            ambient::bind([&val](typename iterator::block_type& block, size_t first, size_t second){
    ...
    void generate(ForwardIterator begin, ForwardIterator end, Generator gen){
        typedef block_iterator<typename ForwardIterator::container_type> iterator;
        for(iterator bit(begin,end); bit != end; ++bit){
            ambient::bind([gen](typename iterator::block_type& block, size_t first, size_t second){
                   ...
                   BinaryOperation binary_op)
    {
        ...
        for(iterator bit(begin1,begin2,result,end1-begin1); bit != end1; ++bit){
            ambient::bind([binary_op](const typename iterator::template get_block_type<0>& block1, size_t first1, size_t second1,
                   ...
                   OutputIterator result, UnaryOperation op)
    {
        ...
        for(iterator bit(begin,result,end-begin); bit != end; ++bit){
            ambient::bind([op](const typename iterator::template get_block_type<0>& block1, size_t first1, size_t second1,
    ...
    OutputIterator copy(InputIterator begin, InputIterator end, OutputIterator result){
        ...
        for(iterator bit(begin,result,end-begin); bit != end; ++bit){
            ambient::bind([](const typename iterator::template get_block_type<0>& block1, size_t first1, size_t second1,
                                   typename iterator::template get_block_type<1>& block2, size_t first2){
    ...
    void replace(ForwardIterator begin, ForwardIterator end, T old_value, T new_value){
        typedef block_iterator<typename ForwardIterator::container_type> iterator;
        for(iterator bit(begin,end); bit != end; ++bit){
            ambient::bind([old_value,new_value](typename iterator::block_type& block, size_t first, size_t second){
    ...
    ambient::atomic<T> reduce(InputIterator begin, InputIterator end, T init){
        ...
        for(size_t b = 0; bit != end; ++bit){
            ambient::bind([](const block_type& block, size_t first, size_t second, const block_type& res, size_t idx){
        ...
        ambient::bind([](const block_type& block, ambient::atomic<T>& value){
                          const typename block_type::value_type* array = block.cbegin();
    ...
    ambient::atomic<T> reduce(InputIterator begin, InputIterator end, T init, BinaryOperation op){
        ...
        for(size_t b = 0; bit != end; ++bit){
            ambient::bind([op](const block_type& block, size_t first, size_t second, const block_type& res, size_t idx){
                              const typename block_type::value_type* array = block.cbegin();
        ...
        ambient::atomic<T> res(init);
        ambient::bind([op](const block_type& block, ambient::atomic<T>& value){
                          const typename block_type::value_type* array = block.cbegin();
                          while(!ambient::locked_once(block)) continue;
    ...
    ambient::atomic<InputIterator> find(InputIterator begin, InputIterator end, const T& val){
        ...
        for(size_t b = 0; bit != end; ++bit){
            size_t offset = bit.offset();
            ambient::bind([val,offset](const block_type& block, size_t first, size_t second, const ambient::vector<int>& res, size_t idx){
                              const typename block_type::value_type* array = block.cbegin();
        ...
        ambient::bind([val,begin](const ambient::vector<int>& positions, ambient::atomic<InputIterator>& match){
    ...
    void remove(InputIterator begin, InputIterator end, const T& val){
        ...
        for(iterator bit(begin,end); bit != end; ++bit){
            ambient::bind([val](block_type& block, size_t first, size_t second){
    ...
    void unique(InputIterator begin, InputIterator end, BinaryPredicate pred){
        ...
        for(iterator bit(begin,end); bit != end; ++bit){
            ambient::bind([pred](block_type& block, size_t first, size_t second){
